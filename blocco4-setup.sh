#!/bin/bash
# BLOCCO 4 — Esegui tutto in un colpo sul VPS
# cd ~/risparmio-energetico && bash <(cat questo file)

# 1. File istruzioni agente
cat > ~/risparmio-energetico/OPENCLAW.ISTRUZIONIENERGIA.md << 'EOFI'
...
echo "✅ OPENCLAW.ISTRUZIONIENERGIA.md creato"

# 2. Fix testo banner cookie (niente riferimento Amazon)
sed -i 's/cookie analitici e di profilazione/cookie analitici/g' ~/risparmio-energetico/layouts/partials/extend_footer.html
sed -i 's/cookie analitici e di profilazione/cookie analitici/g' ~/risparmio-energetico/layouts/partials/extend_head.html
sed -i 's/Cookie di Affiliazione Amazon/Cookie di Marketing/g' ~/risparmio-energetico/layouts/partials/extend_footer.html
sed -i 's/Per tracciare i clic sui link affiliati e guadagnare commissioni./Per mostrare contenuti personalizzati e misurare le performance./g' ~/risparmio-energetico/layouts/partials/extend_footer.html
echo "✅ Banner cookie adattato"

# 3. Prima build Hugo
cd ~/risparmio-energetico
hugo --minify
echo "✅ Hugo build completato"

# 4. Primo commit
git add .
git commit -m "Init sito risparmio energetico - struttura base"
echo "✅ Commit creato"
echo ""
echo "🏁 Prossimo step: git remote add origin https://github.com/SalvatoreIp/NOME-REPO.git && git push -u origin main"
